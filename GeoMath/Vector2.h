...
struct Vector2
{
    ...
    float   angle()     const { return atan2f(y,    x)/ADJUST; }

    Vector2 normal()    const { return Vector2{x, y} /= magnitude(); }
    Vector2 operator-() const { return { -x,-y }; }
    Vector2 perp()      const { return { -y, x }; }

    static Vector2 fromAngle(float a) { return{ cosf(a*ADJUST),sinf(a*ADJUST) }; }
};
...
inline Vector2 reflect(const Vector2 &incident, const Vector2 &normal) { return incident - normal * 2 * dot(incident, normal); }

// normal should be normalized
inline Vector2 project(const Vector2 &incident, const Vector2 &normal) {  return normal * dot(incident, normal); }